...
[![Install on Greasy Fork](https://img.shields.io/badge/Install-Greasy%20Fork-red?logo=tampermonkey)](https://greasyfork.org/scripts/567163-dungeon-floor-timer)
...
65      �?       �?       �?      �?     �?Total   19m 25s  21m 13s  -1m 47s 3      4.3
...
| Sinister Circus / 阴森马戏�? | 60     | Deranged Jester | Zombie Bear (40+)                                  |
...
2. Install from [Greasy Fork](https://greasyfork.org/scripts/567163-dungeon-floor-timer) or create a new script and paste [`dungeon-floor-timer.user.js`](dungeon-floor-timer.user.js)
...
银河奶牛放置（Milky Way Idle）地牢计时器插件，追踪每5层的通关时间，支�?**Speedrun风格对比** �?**额外Boss刷新统计**�?
自动检测游戏语言，支�?*中英文动态切�?*�?
---
...
### �?�?层分组计�?
自动�?层分组计时（1-5, 6-10, ...），最后一层Boss单独一行显示�?
### 📊 Speedrun对比
...
- **中途进�?*：等待下一个完�?层组才开始计时，不完整轮不计入历�?- **仅保存完整轮**：只有从�?波开始的完整轮才存入历史
- **持久化存�?*：完整轮数据保存�?localStorage，刷新页面不丢失
- **可拖动面�?*：随意拖动位置，支持收起/展开
...
```
�?地牢计时�?                            [重置] [收起]
海盗基地  �?65/65  已用 19m 25s

层数    用时     均时     对比     额外  �?1-5     47s      52s      -4s      0     0.0
6-10    1m 20s   1m 16s   +3s      0     0.0
11-15   1m 11s   1m 16s   -4s      0     0.0
16-20   1m 5s    1m 0s    +4s      0     0.0
21-25   1m 44s   1m 41s   +2s      0     0.0
...
56-60   1m 57s   2m 3s    -5s      1     1.0
61-64   1m 11s   1m 35s   ...      1     0.3
65      �?       �?       �?       �?    �?总计    19m 25s  21m 13s  -1m 47s  3     4.3
...
1. 安装 [Tampermonkey](https://www.tampermonkey.net/) 浏览器扩�?2. �?[Greasy Fork](https://greasyfork.org/scripts/567163-dungeon-floor-timer) 安装，或新建脚本粘贴 [`dungeon-floor-timer.user.js`](dungeon-floor-timer.user.js)
3. 刷新游戏页面

> ⚠️ 脚本必须�?`document-start` 运行以拦�?WebSocket 消息�?
---

## 工作原理

通过包装游戏�?WebSocket 连接拦截消息�?
| 消息类型                | 用�?                                 |
|-------------------------|---------------------------------------|
| `new_battle`            | 获取波次号和怪物列表（计�?+ Boss检测） |
| `init_character_data`   | 检测当前地�?                         |
